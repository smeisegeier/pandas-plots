...
# def get_duckdb_filter_n(con=None, query=None, filters=None, debug=False, max_bar_length=30):
#     """
#     Executes a series of cascading queries on a DuckDB connection,
#     calculates row counts and percentages, and optionally visualizes them
#     using a right-aligned ASCII scale.
...
#         con.execute(f"INSERT INTO users SELECT i, CASE WHEN i <= {EXAMPLE_COUNT * 0.3} THEN 25 WHEN i <= {EXAMPLE_COUNT * 0.7} THEN 45 ELSE 60 END, CASE WHEN i % 3 = 0 THEN 'inactive' ELSE 'active' END, CASE WHEN i <= {EXAMPLE_COUNT * 0.5} THEN 'east' ELSE 'west' END FROM range(1, {EXAMPLE_COUNT + 1}) tbl(i);")
...
#     base_query = f"SELECT count(*) FROM ({base_query_source})"
...
#     MAX_LEFT_TEXT_WIDTH = max_caption_len + len("    - []:")
    
#     # 2. Determine the fixed width for the percentage/count display. 
    
#     # WIDTH_COUNT_PART must account for the "n = " prefix
...
#     # PADDING CALCULATION:
#     # 1. Length of the entire filter line prefix (caption block, used for alignment):
#     #    MAX_LEFT_TEXT_WIDTH
#     # 2. Length of the text being printed on the initial line:
#     #    len(n_part)
    
#     # The padding needed between n_part and initial_percent is the difference between the 
#     # filter line's total prefix length and the initial line's starting text length.
...
#         full_query = f"SELECT count(*) FROM ({base_query_source}) WHERE {current_where_clause}"
...
#         # Left Text: Caption, padded to MAX_LEFT_TEXT_WIDTH
#         left_text = f"    - [{caption}]:"
...
def get_duckdb_filter_n(con=None, query=None, filters=None, debug=False, max_bar_length=30):
    ...
    BLOCK_FILLED = "█" 
    BLOCK_EMPTY = "░"  
    BOX_CORNER = "└" # New character for filter lines
    ...
    # The prefix for the filter lines is now '└ [Caption]:' (length: 1 + 1 + len([]) + 1)
    # The longest filter line will now determine the necessary padding.
    ...
    # PADDING: The padding needed is the space created by MAX_LEFT_TEXT_WIDTH (caption block) 
    # plus the space taken by the count block (WIDTH_COUNT_PART).
    
    # Padding needed between n_part and initial_percent is the space defined by the filter line's
    # prefix (MAX_LEFT_TEXT_WIDTH) plus the count block's width (WIDTH_COUNT_PART).
    padding_needed = MAX_LEFT_TEXT_WIDTH + WIDTH_COUNT_PART - len(n_part)
